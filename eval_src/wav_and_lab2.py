...
def _plot_discreate_sequence(true_data, title, sample_data, wrd_idx, cmap=None, cmap2=None, label_cmap=None):
    ...
    # plot label data
    ax = plt.subplot2grid((10, 1), (0, 0), rowspan = 2)
    ...
    ylim = ax.get_ylim()
    plt.xticks(())
    plt.ylabel('Wave\n form\n') #, rotation="horizontal", labelpad=22)
    #^# change position #^#
    
    
    # plot label data
    ax = plt.subplot2grid((10, 1), (2, 0))
    ax.axis("off")
    plt.sca(ax)
    ...
    if label_cmap is None:
        label_cmap = cmap
    ax.matshow([true_data], aspect='auto', cmap=label_cmap) #, alpha=0.5
    plt.ylabel('Truth\n label', rotation=0, labelpad=22)
    """

    ax = plt.subplot2grid((10, 1), (3, 0), rowspan = 6)
    #plt.suptitle(title)
    plt.sca(ax)
    ...
    plt.xlabel(str(wrd_idx_int), size=14)
    plt.xticks(())
    ...
    ylim = ax.get_ylim()
    #print(xlim)
    ax.text(xlim[1]/2,-14,"Frame", ha="center", size=14)
    
    ax.matshow([true_data], aspect='auto', cmap=label_cmap, extent=[*xlim, *ylim], alpha=0.5) #, alpha=0.5
...
for i, name in enumerate(tqdm(names)):
    ...
    plt.savefig(result_target+"figures/wav_lab_" + name + ".eps", bbox_inches="tight")
    plt.savefig(result_target+"figures/wav_lab_" + name + ".png", bbox_inches="tight")
    plt.clf()
