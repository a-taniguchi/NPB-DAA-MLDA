...
def word_weight_set(flag):
    ...
    elif flag == "low": word_weight = 20
    elif flag == "decrease": 
        weight_tmp = 300-(iter*10) # bugfix by akira
        if iter <= 10:
            word_weight = 200
        else:
            word_weight = min( max(20, weight_tmp), 200 ) # akira
    else:
...
if cont == 0:
    ...
    for _ in trange(L): #copy L candidates
        ...
        for t in range(pretrain_iter):
            letter_hsmm.resample_model(num_procs=thread_num) #0
